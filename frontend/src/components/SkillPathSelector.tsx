...
interface SkillPathSelectorProps {
  skillTitle: string;
  // ✅ Changed from `paths` to `path_items`
  path_items: SkillPath[];
  onPathSelect: (pathId: string, language: string) => void;
}

const SkillPathSelector = ({
  skillTitle,
  path_items,
  onPathSelect,
}: SkillPathSelectorProps) => {
  ...
  // ✅ Safe default if no paths exist
  const pathsToRender = path_items || [];
  ...
  return (
    <div className="space-y-6">
      <div className="text-center animate-fade-in">
        ...
        <p
          className="text-gray-600 dark:text-gray-300 mb-6 animate-fade-in"
          style={{ animationDelay: "0.2s" }}
        >
          Select the learning path that best fits your goals and experience
          level
        </p>
        ...
        {pathsToRender.map((path, index) => (
          ...
          >
            <CardHeader className="text-center">
              <div className="text-4xl mb-3 group-hover:scale-110 group-hover:rotate-12 transition-all duration-300">
                {path.icon}
              </div>
              <CardTitle className="group-hover:text-blue-600 transition-colors text-lg">
                {path.title}
              </CardTitle>
              <CardDescription className="text-sm group-hover:text-gray-700 dark:group-hover:text-gray-200 transition-colors duration-300">
                {path.mini_desc}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div
                className="bg-blue-50 dark:bg-gray-700 p-3 rounded-lg hover:bg-blue-100 dark:hover:bg-gray-600 transition-colors duration-300 animate-fade-in"
              ...
              <Button
                ...
                onClick={() => onPathSelect(String(path.id), selectedLanguage)}
